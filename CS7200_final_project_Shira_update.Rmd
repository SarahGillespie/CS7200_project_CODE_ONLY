...
part_5<- data %>% 
  ...
  filter(!PARTICIPANT %in% c('CR10', 'CR34')) %>% #filter out those who did not contribute to the full month
...
part_521_agg <- part_2021 %>% 
  group_by(PARTICIPANT, half, group) %>% 
  summarize(mean_upright = mean(upright_prop),
            mean_ambulation = mean(ambulation_prop)) # 6 participants

part_522_agg <- part_2022 %>% 
  group_by(PARTICIPANT, half, group) %>% 
  summarize(mean_upright = mean(upright_prop),
            mean_ambulation = mean(ambulation_prop)) # 10 participants
...
wilcox.test(part_2021$ambulation_prop, part_2022$ambulation_prop) # p-value < 0.05
...
part_3 <- data %>% 
  ...
  mutate(PARTICIPANT = as.factor(PARTICIPANT),
         ...
         half = as.factor(case_when(day <= 15 ~ 1, TRUE ~ 2)), # split by 1st and 2nd half of month
         half = as.factor(half)) %>% 
  filter(month == "03") %>% # filter to month of May only
  #filter(!PARTICIPANT %in% c('CR10', 'CR34')) %>% #filter out those who did not contribute to the full month
  select(PARTICIPANT, group, half, year, upright_prop, ambulation_prop, HOURS_COVERAGE)
...
fit5 <- lmer(upright_prop ~ group + half + (1|PARTICIPANT), part_5, REML=F)
fit6 <- lmer(ambulation_prop ~ group + half + (1|PARTICIPANT), part_5, REML=F)
